...
function createWindow () {
	...
	mainWindow = new BrowserWindow({
		...
		resizable: true,
		center: true,
		icon: path.join(__dirname, 'ITPharmaSQLViewer.ico'),
		skipTaskbar: false,
		webPreferences: {
...
mainMenu = Menu.buildFromTemplate([
			...
			},
			{
				...
				click: function(){
					dialog.showSaveDialog(mainWindow, {
						...
						filters: [
							{ name: 'Файлы настроек (*.itpsql)', extensions: ['itpsql'] },
	...
	},
	{
		...
		submenu: [
			{
				label: 'Запустить SSMS',
				click: function(){
					mainWindow.webContents.send('execSQLViewer');
				}
			},
			{
				label: 'О программе',
				click: function(){
					mainWindow.webContents.send('aboutSQLViewer');
				}
			}
		]
	}
...
ipcMain.on('openSqlPathDialog', function(event, arg){
	if(typeof(arg) === 'string'){
		dialog.showOpenDialog(mainWindow, {
			title: 'Выберите путь к Microsoft SQL Server Management Studio (Ssms.exe)',
			defaultPath: arg,
			filters: [
				{ name: 'Microsoft SQL Server Management Studio (*.exe)', extensions: ['exe'] },
				{ name: 'Все файлы', extensions: ['*'] }
			],
			properties: ['openFile']
		}).then(function(res){
			if(Array.isArray(res.filePaths) && (res.filePaths.length > 0)) { 
				mainWindow.webContents.send('openSqlPath', [res.filePaths[0]]); 
			}
		}).catch(function(err){});
	}
});